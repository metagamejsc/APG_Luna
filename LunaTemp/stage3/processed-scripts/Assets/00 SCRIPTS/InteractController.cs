...
public class InteractController : MonoBehaviour
{
    ...
    private void ActionOnClick(GameObject item)
    {
        if (lockPlay) return;
        if (tut)
        {
        ...
        if (itemClick.UnActiveObject) itemClick.UnActiveObject.SetActive(false);
    }
    private void DropItem(GameObject dropArea)
    {
        ...
        AudioController.Ins.PlaySFX(currentTarget.Sound);
...
public struct ItemClick
{
    ...
    public bool IsEmpty => !ClickArea || !Item;
}
